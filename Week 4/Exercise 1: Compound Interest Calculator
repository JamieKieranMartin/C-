...
namespace CompoundInterestCalculator
{
    public class CompoundCalculator
    {
        ...
        {
            Console.WriteLine("The final amount of money plus interest is: {0}", finalAmount.ToString("C"));
        ...
        public static double CalculateInterest(int menuOption)
        {
            ...
            if (menuOption == CALCULATE_DAILY || menuOption == CALCULATE_QUARTERLY)
            {
                ...
                {
                    finalAmount = CalculateCompoundInterest(principal, interestRate, numYears, 0.25);
                }
            }
            else
            {
        ...
        public static double CalculateCompoundInterest(double principal, double interestRate, double numYears)
        {
            double output;
            double outputBase = 1 + (interestRate / 365);
            double outputExponent = 365 * numYears;
            output = principal * Math.Pow(outputBase, outputExponent);
            return output;
        }
        ...
        public static double CalculateCompoundInterest(double principal, double interestRate, int numYears, double compoundRate)
        {
            double output;
            double outputBase = 1 + (interestRate / compoundRate);
            double outputExponent = compoundRate * numYears;
            output = principal * Math.Pow(outputBase, outputExponent);
            return output;
        }
